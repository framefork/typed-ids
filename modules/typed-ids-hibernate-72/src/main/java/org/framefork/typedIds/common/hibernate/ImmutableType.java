    ...
    TypeConfigurationAware
{
    ...
    {
        return getJdbcType().getExtractor(getExpressibleJavaType()).extract(rs, position, options);
    }
    ...
    {
        getJdbcType().getBinder(getExpressibleJavaType()).bind(st, value, index, options);
    }
    ...
    {
        return "%s backed by %s".formatted(javaType, getJdbcType());
    }
